...
function validateBackendDestinationScenario (req, cb){
  if (process.env.NODE_ENV === 'development') { // in order to ensure that test will not enter here in case tenantmode is shared (and the test needs it to be shared)
  ...
  uaaUtils.getUaaConfig(req, function (err, uaaOptions) {
    ...
    if (!req || !req.internalUrl || !req.internalUrl.route || !req.internalUrl.route.destination || !req.internalUrl.destination
      || (req.internalUrl.destination.authentication !== 'OAuth2SAMLBearerAssertion' && uaaOptions.tenantmode !== 'shared')) {
      return cb(null, false);
    }
